...
export default function CountdownTimer({ lang = 'es' }: { lang?: 'es' | 'en' }) {
        ...
        <div className="mb-6 p-4 bg-[#d4af37]/5 border border-[#d4af37]/20 rounded-lg animate-fade-in">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                    <p className="text-[11px] uppercase tracking-wider text-[#d4af37] font-bold mb-1">
                        {lang === 'en' ? 'SHIPS TODAY' : 'Envío Hoy Mismo'}
                    </p>
                    <p className="text-xs text-gray-400 font-medium">
                        {lang === 'en' ? 'Order in the next:' : 'Ordena en los próximos:'}
                    </p>
                </div>
                ...
                <div className="flex gap-2">
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[8px] uppercase text-gray-500 mt-1">{t('product.hours')}</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[8px] uppercase text-gray-500 mt-1">{t('product.minutes')}</span>
                    </div>
                    <span className="text-lg font-bold text-[#d4af37] pt-0.5">:</span>
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[8px] uppercase text-gray-500 mt-1">{t('product.seconds')}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-3 flex items-center gap-2 py-2 px-3 bg-black/20 rounded border-t border-white/5">
                ...
                <span className="text-[10px] text-gray-300">
                    {lang === 'en' ? (
                        <>Your jewelry will be processed with <strong>VIP priority</strong> today.</>
                    ) : (
